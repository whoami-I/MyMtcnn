...
def generate_bounding_box(cls_pro,bbox_pred,scale,threshold):
    ...
    #将x1Arr, y1Arr, x2Arr, y2Arr, score, bbox_pred拼在一起，变成1维
...
def detect_pent(image):
    ...
    boxes_c = np.concatenate([x1Arr.reshape(-1, 1),
                             ...
                             axis=-1)
    # boxes是多行4列的矩阵，四列数据分别表示在current_scale下的image的人脸框
    # boxes_c是多行5列矩阵，前四列是经过矫正后的在current_scale下的image的人脸框，第五列表示人脸得分框
    return boxes,boxes_c

'''
    P-Net识别出一系列框之后，从原图中裁剪出对应的img，resize成24*24大小输入到R-Net中，
    因此R-Net的输入不是人脸金字塔，输出来结果之后，就是对P-Net框的一个矫正
'''
