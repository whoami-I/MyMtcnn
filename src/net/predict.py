...
def processed_img(img, scale):
    ...
    n_w = int(w * scale)
    dsize = (n_w, n_h)
    img_resized = cv.resize(np.array(img), dsize, interpolation=cv.INTER_LINEAR)
...
def detect_pnet(img, model):
    net_size = 12
    scale_factor = 0.709
    current_scale = float(net_size) / min_face_size
    im_resized = processed_img(img, current_scale)
    all_boxes = list()
...
def detect_Rnet(model, img, dets):
    '''通过rent选择box
    参数：
      im：输入图像
    ...
    for i in range(num_boxes):
        ...
        cropped_imgs[i, :, :, :] = (cv.resize(tmp, (24, 24)) - 127.5) / 128
...
def detect_Onet(model, img, dets):
    """
    ...
    for i in range(n_boxes):
        tmp = np.zeros((dh[i], dw[i], 3), dtype=np.uint8)
        tmp[dy[i]:edy[i] + 1, dx[i]:edx[i] + 1, :] = img[y[i]:ey[i] + 1, x[i]:ex[i] + 1, :]
        cropped_imgs[i, :, :, :] = (cv.resize(tmp, (48, 48)) - 127.5) / 128
...
if __name__ == '__main__':
    ...
    r_model = keras.models.load_model('r_model')
    o_model = keras.models.load_model('o_model')
    cv.namedWindow('im', cv.WINDOW_NORMAL)
    # cv.resizeWindow('im', 600, 600)
    for f in files:
        im = cv.imread('image\\' + f)
        # im = cv.cvtColor(im, cv.COLOR_BGR2RGB)
        boxes, boxes_c = detect_pnet(im, p_model)
        r_boxes, r_boxes_c = detect_Rnet(r_model, im, boxes_c)
        o_boxes, o_boxes_c = detect_Onet(o_model, im, r_boxes_c)
        for i in range(o_boxes_c.shape[0]):
            bbox = o_boxes_c[i, :4]
            score = o_boxes_c[i, 4]
            corpbbox = [int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])]
            # 画人脸框
            cv.rectangle(im, (corpbbox[0], corpbbox[1]),
                         (corpbbox[2], corpbbox[3]), (0, 0, 255), 1)
        img = cv.resize(im, (1000, int(1000 * im.shape[0] / im.shape[1])))
        cv.imshow('im', img)
        cv.waitKey(0)
